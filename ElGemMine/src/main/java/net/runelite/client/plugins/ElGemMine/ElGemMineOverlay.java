...
@Slf4j
@Singleton
class ElGemMineOverlay extends OverlayPanel
{
	private final ElGemMinePlugin plugin;
	private final ElGemMineConfiguration config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private ElGemMineOverlay(final Client client, final ElGemMinePlugin plugin, final ElGemMineConfiguration config)
	{
		super(plugin);
		setPosition(OverlayPosition.DYNAMIC);
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "gem mine overlay"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startGemMine || !config.enableUI())
		{
