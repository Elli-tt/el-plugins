...
@Slf4j
@Singleton
class ElTeaksOverlay extends OverlayPanel
{
	private final ElTeaksPlugin plugin;
	private final ElTeaksConfiguration config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private ElTeaksOverlay(final Client client, final ElTeaksPlugin plugin, final ElTeaksConfiguration config)
	{
		super(plugin);
		setPosition(OverlayPosition.DYNAMIC);
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "teaks overlay"));
	}
