...
@Slf4j
@Singleton
class ElPlanksOverlay extends OverlayPanel
{
    private final ElPlanksPlugin plugin;
    private final ElPlanksConfig config;

    String timeFormat;

    @Inject
    private ElPlanksOverlay(final Client client, final ElPlanksPlugin plugin, final ElPlanksConfig config)
    {
        super(plugin);
    ...
    {
        if (plugin.botTimer == null || !plugin.startPlanks || !config.enableUI())
        {
